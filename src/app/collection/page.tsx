...
export default function CollectionLibraryPage() {
  ...
  const [activeTab, setActiveTab] = useState<'All' | 'Grouped by Sender' | 'Grouped by Subject' | 'Grouped by Matter'>('Grouped by Sender');
  ...
  const groupedDocs = useMemo(() => {
    if (activeTab === 'All') return { 'All Files': filteredDocs };
  ...
  return (
    <div style={{ padding: '40px 48px', maxWidth: 1400, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 24, borderBottom: '1px solid var(--border-default)', paddingBottom: 16 }}>
        {['Grouped by Sender', 'Grouped by Subject', 'Grouped by Matter', 'All'].map(tab => (
          <button
            key={tab}
              ...
              <div style={{ padding: '12px 20px' }}>
                {docs.map((doc, idx) => (
                  <div key={idx} style={{
                    ...
                    borderBottom: idx === docs.length - 1 ? 'none' : '1px solid var(--border-default)'
                  }}>
                    {/* Icon */}
                    <div style={{
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, fontSize: 12, color: 'var(--text-muted)' }}>
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                          <span style={{ width: 6, height: 6, borderRadius: '50%', background: doc.source.includes('Google') ? '#16A34A' : doc.source.includes('Mail') ? '#DC2626' : '#4F46E5' }} />
                          {doc.source}
                        </span>
                        <span>•</span>
                        <span>{doc.size}</span>
